...
if(!empty($gap) && $gap == 'fornecedor') {
    modal_link('NOVO FORNECEDOR', 'add');
    br();
    ptable('Fornecedores');

} else {
    modal_link('NOVO CLIENTE', 'add');
    br();
    ptable('Clientes');
...
if(!empty($gap) && $gap == 'fornecedor') {
    omodal('Adicionar Fornecedor', 'add', 'modal-lg');
} else {
    omodal('Adicionar Cliente', 'add', 'modal-lg');
}

form_open('crm/empresas');
...
cdiv();
col(3);
form_checkbox('Agência?', 'agencia', 1);
cdiv();
col(3);
form_checkbox('Cliente?', 'cliente', 1, 1);
cdiv();
col(3);
...
cdiv();
col(2);
form_select2_data_blank('Tipo', 'tipop', $tipo, 'nome');
cdiv();
col(8);
