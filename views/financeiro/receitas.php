...
omodal('Novo movimento', 'add', 'modal-lg');
    form_open('financeiro/receitas', 'POST');
   ...
   cmodal();
    ...
    div('pull-right');
        ...
        omodal('FILTRO FINANCEIRO', 'filtrar');
            form_open('financeiro/receitas', 'get');
                row();
                    col(6);
                        $status = [['value' => 'aberto', 'nome' => 'Aberto'], ['value' => 'pago', 'nome' => 'Pago']];
...
row();
    col(3);
        // Total geral
        dashboard_count('TOTAL DAS MOVIMENTAÇÕES', 'R$ ' . moeda_real($tgeral), 'fa fa-money','financeiro/receitas');
    
    cdiv();
    col(3);
        // Total em pago
        dashboard_count('TOTAL PAGO', 'R$ ' . moeda_real($tpago), 'fa fa-money','financeiro/receitas');
    cdiv();
    col(3);
        // Em atraso
        dashboard_count('TOTAL ATRASADO', 'R$ ' . moeda_real($tatraso), 'fa fa-money','financeiro/receitas');
    cdiv();
    col(3);
        // Em atraso
        dashboard_count('TOTAL ABERTO', 'R$ ' . moeda_real($tgeral - $tpago), 'fa fa-money','financeiro/receitas');
    cdiv();
cdiv();

ptable('Movimentos');

datatable('movimentos', ['Código', 'Tipo', 'Data de Inclusão', 'Título', 'Vencimento', 'Valor', 'Status'], ['id','tipo', 'data_lancamento','titulo','data_vencimento','valor','status'], $movimentos, [
