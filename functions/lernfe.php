...
function importaNFe($xml){
    ...
    $dados['dataEmissaoH'] = substr(tagValue($doc,"dhEmi"), 0, 10);
    ...
    // Totais da NF-e. Para fazer a alimenta��o no database:
...
/*
    ...
    // Echo nos valores totais da NF. Valor do PIS e COFINS deve ser calculado no c�digo fonte de acordo com o c�lculo do e-mail do Renan:
    $calculo_sem_ipi_e_st = $dados['valortotalnf'] - $dados['valoripi'] - $dados['valorst'] + $dados['valorfrete'] ;
    
    //Calcula PIS e COFINS da nota fiscal em quest�o - USA A VARIAVEL PADRAO ACIMA:
    $resultado_pis = ($calculo_sem_ipi_e_st * 0.65)/ 100;
    $resultado_cofins = ($calculo_sem_ipi_e_st * 3) / 100;

// INSER��O DE DADOS NO MYSQL:
...
// Saida dos dados conforme a fun��o importaNFE e alimenta��o no MySQL:
